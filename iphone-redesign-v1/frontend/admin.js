...
const ADMIN_API_BASE_URL = adminUrlParameters.get("apiBaseUrl") || "http://127.0.0.1:8000";
...
function renderAdminPanelVisibility() {
  ...
  document.getElementById("admin-account-panel").classList.toggle("hidden", !isAdmin);
...
function renderAdminGroupsList() {
  ...
  container.innerHTML = adminState.groups.map(function (group) {
    return '<article class="config-item">' +
      ...
      '<label><input id="admin-group-can-approve-' + group.id + '" type="checkbox"' + (group.can_approve ? " checked" : "") + "> Approval group</label>" +
      "</div>" +
      '<div class="form-row checkbox-row">' +
      '<label><input id="admin-group-required-' + group.id + '" type="checkbox"' + (group.approval_required ? " checked" : "") + "> Required for approval</label>" +
      "</div>" +
...
function renderAdminUsersList() {
  ...
  container.innerHTML = adminState.users.map(function (user) {
    ...
    return '<article class="config-item">' +
      ...
      (isCurrentAdmin
        ? '<span class="small-text">Use the account section for your own password.</span>'
...
async function handleAdminPasswordSubmit(event) {
  ...
  try {
    await adminApiRequest("/me/password", {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    showAdminMessage("Password updated successfully.", "success");
  } catch (error) {
    showAdminMessage(error.message, "error");
  }
}
