...
def delete_area(area_id: int, authorization: Optional[str] = Header(default=None)):
    ...
    with get_connection() as connection:
        ...
        connection.execute("DELETE FROM bookings WHERE area_id = ?", (area_id,))
...
def create_booking(payload: BookingCreateRequest, authorization: Optional[str] = Header(default=None)):
    ...
    if not area_exists(payload.area_id):
    ...
    with get_connection() as connection:
        if status == "planned":
            ensure_booking_has_no_blocking_overlap(
                connection,
        ...
        write_audit_log(
            ...
            "booking",
            booking_id,
            {"status": status, "owner_group_id": owner_group_id},
        )
        connection.commit()
        booking_row = fetch_booking_by_id(connection, booking_id)
...
def update_booking(
    ...
    with get_connection() as connection:
        ...
        area_or_time_changed = (
            booking_row["area_id"] != payload.area_id
        ...
        write_audit_log(
            ...
            "booking",
            booking_id,
            {"status": next_status, "approvals_cleared": approvals_cleared},
        )
        connection.commit()
...
def ensure_requested_booking_can_be_approved(connection, booking_row) -> None:
    if has_approval_blocking_overlap(connection, booking_row):
        raise HTTPException(
            status_code=400,
            detail="This requested booking cannot be approved because the area already has an overlapping planned or approved booking.",
        )
...
def ensure_booking_has_no_blocking_overlap(
    connection,
    area_id: int,
    ...
    if excluded_booking_id is None:
        conflicting_booking = connection.execute(
            """
            SELECT id
            FROM bookings
            ...
            LIMIT 1
            """,
            (area_id, end_time_value, start_time_value),
        ).fetchone()
    else:
        conflicting_booking = connection.execute(
            """
            ...
            LIMIT 1
            """,
            (area_id, excluded_booking_id, end_time_value, start_time_value),
        ).fetchone()

    if conflicting_booking:
        raise HTTPException(
            status_code=400,
            detail="This area already has an overlapping planned or approved booking in that time range.",
        )
