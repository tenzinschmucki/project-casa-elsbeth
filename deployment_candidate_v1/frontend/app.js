...
const runtimeAppConfig = window.APP_CONFIG || {};

if (shouldRedirectFromLocalFile()) {
  window.location.replace(getServedPreviewUrl());
}

const API_BASE_URL = urlParameters.get("apiBaseUrl") || runtimeAppConfig.apiBaseUrl || getDefaultApiBaseUrl();
...
const state = {
  ...
  mobileBookingsScope: "all",
  mobileHideArchived: true,
  mobileApprovalsScope: "pending",
  language: "en",
  filters: {
    areaId: "",
    status: "",
...
const ICONS = {
  ...
  bookings: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 7.5h10M7 12h10M7 16.5h10" fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="1.8"></path><circle cx="4.5" cy="7.5" r="1" fill="currentColor"></circle><circle cx="4.5" cy="12" r="1" fill="currentColor"></circle><circle cx="4.5" cy="16.5" r="1" fill="currentColor"></circle></svg>',
  create: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5.2v13.6M5.2 12h13.6" fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="1.9"></path></svg>',
  more: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="5.5" cy="12" r="1.6" fill="currentColor"></circle><circle cx="12" cy="12" r="1.6" fill="currentColor"></circle><circle cx="18.5" cy="12" r="1.6" fill="currentColor"></circle></svg>',
  chevron: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 6 6 6-6 6" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.9"></path></svg>',
  alert: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 4.8 20 18.2H4L12 4.8Z" fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="1.8"></path><path d="M12 9.4v4.6M12 17.1h.1" fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="1.8"></path></svg>'
};
...
function renderMobileBottomNav() {
  ...
  buttons.forEach(function (button) {
    ...
    const label = isGuest && viewName === "more" ? "Login" : capitalizeWord(viewName);
    const iconName = getMobileNavIconName(viewName);

    button.classList.toggle("is-active", isActive);
    button.classList.toggle("is-disabled", isDisabled);
    button.classList.toggle("hidden", shouldHideForGuest);
    button.classList.toggle("mobile-nav-button-create", viewName === "create");
    button.disabled = isDisabled;
    button.setAttribute("aria-pressed", String(isActive));
    button.innerHTML = '<span class="mobile-nav-icon">' + renderIcon(iconName) + '</span><span class="mobile-nav-label">' + escapeHtml(label) + '</span>';
  });
}
...
function renderMobileCalendarScreen() {
  ...
  return '<section class="mobile-screen">' +
    '<section class="mobile-calendar-hero">' +
    '<div class="mobile-calendar-title-row">' +
    '<div class="mobile-calendar-title-block">' +
    '<h2>Calendar</h2>' +
    '</div>' +
    '<div class="view-switch mobile-mode-switch" aria-label="Calendar view">' +
    ...
    '</div>' +
    '</div>' +
    '<section class="mobile-period-toolbar">' +
    '<div class="mobile-period-pill">' + escapeHtml(getCurrentPeriodLabelText()) + '<span class="inline-icon">' + renderIcon("chevron") + '</span></div>' +
    '<div class="mobile-period-actions">' +
    '<button type="button" class="secondary-button mobile-period-action" data-mobile-today="true">Today</button>' +
    '<button type="button" class="secondary-button icon-button mobile-period-circle" data-mobile-period="-1" aria-label="Previous period">&#x2039;</button>' +
    '<button type="button" class="secondary-button icon-button mobile-period-circle" data-mobile-period="1" aria-label="Next period">&#x203A;</button>' +
    '</div>' +
    '</section>' +
    '<section class="panel mobile-legend-strip">' +
    renderMobileStatusLegend() +
    '</section>' +
    '</section>' +
    '<section class="panel mobile-calendar-panel mobile-calendar-panel-' + escapeHtml(state.viewMode) + '">' +
...
function renderMobileBookingsScreen() {
  const bookings = getMobileBookingsList();
  let markup = '<section class="mobile-screen">' +
    renderMobilePrimaryHeader(
      "Bookings",
      '<div class="mobile-bookings-header-actions">' +
      renderMobileScopeSwitch("bookings", state.mobileBookingsScope, [
        { value: "all", label: "All" },
        { value: "me", label: "Me" }
      ]) +
  ...
  if (state.mobileShowFilters) {
    markup += '<section class="panel mobile-control-panel">' + renderMobileFilterPanel() + '</section>';
  }

  markup += '<section class="mobile-card-list">';
...
function renderMobileCreateScreen() {
  ...
  if (!state.currentUser) {
    return '<section class="mobile-screen">' +
      renderMobilePrimaryHeader("Create booking") +
      '<p class="empty-state">Guests can browse only. Open the More tab to log in.</p>' +
      '</section>';
  }

  return '<section class="mobile-screen">' +
    renderMobilePrimaryHeader(editingBooking ? "Modify booking" : "Create booking") +
    '<form id="mobile-booking-form" class="panel mobile-form-panel">' +
    '<div class="mobile-form-grid mobile-form-grid-two mobile-form-grid-header">' +
    ...
    '<p class="small-text form-warning-text">' + escapeHtml(editingBooking && shouldShowResetWarning(editingBooking) ? RESET_WARNING_TEXT : "Changing area or time resets approvals.") + '</p>' +
...
function renderMobileApprovalsScreen() {
  const approvalBookings = getMobileApprovalsList();

  return '<section class="mobile-screen">' +
    renderMobilePrimaryHeader(
      "Approvals",
      renderMobileScopeSwitch("approvals", state.mobileApprovalsScope, [
    ...
    (approvalBookings.length === 0
      ? '<p class="empty-state">' + (state.mobileApprovalsScope === "previous" ? "No previous approvals found for this view." : "Nothing is waiting for your approval right now.") + '</p>'
      : approvalBookings.map(function (booking) {
...
function renderMobileMoreScreen() {
  let markup = '<section class="mobile-screen">' + renderMobilePrimaryHeader(state.currentUser ? "More" : "Login") + '<section class="mobile-card-list">';
  ...
  if (!state.currentUser) {
    ...
    markup += '<section class="panel mobile-more-card">' +
      ...
      '<form id="mobile-password-change-form">' +
      '<div class="form-row"><label for="mobile-current-password">Current password</label><input id="mobile-current-password" name="mobile-current-password" type="password" required></div>' +
      '<div class="form-row"><label for="mobile-new-password">New password</label><input id="mobile-new-password" name="mobile-new-password" type="password" minlength="6" required></div>' +
      '<div class="button-row"><button type="submit">Update password</button></div>' +
      '</form></section>';
  }

  markup += '<section class="panel mobile-more-card"><h3>Language</h3><div class="mobile-inline-actions mobile-inline-actions-split">' +
    renderMobileScopeSwitch("language", state.language, [
      { value: "en", label: "English" },
      { value: "de", label: "Deutsch" }
    ]) +
    '<span class="small-text">Language switching UI is ready for later.</span>' +
    '</div></section>';
...
function renderMobileSelectedDayScreen() {
  ...
  return '<section class="mobile-screen">' +
    renderMobileScreenHeader(formatLongDate(selectedDate), bookings.length + " " + (isGuest ? "occupied" : "booking" + (bookings.length === 1 ? "" : "s")), true, "Selected day") +
    '<section class="mobile-card-list">' +
    ...
    (bookings.length === 0
      ...
      : bookings.map(function (booking) {
        return renderMobileSelectedDayBookingCard(booking);
      }).join("")) +
    '</section></section>';
}
...
function renderMobileSelectedBookingScreen() {
  ...
  if (!booking) {
    return '<section class="mobile-screen">' +
      renderMobileScreenHeader("Selected booking", "No booking selected.", true, "Selected booking") +
...
function renderMobileScreenHeader(title, subtitle, showBack, eyebrowLabel) {
  return '<section class="panel mobile-screen-header">' +
    ...
    '<div class="mobile-screen-title-block">' +
    '<p class="toolbar-label">' + escapeHtml(eyebrowLabel || "Mobile view") + '</p>' +
    '<h2>' + escapeHtml(title) + '</h2>' +
    '<p class="small-text">' + escapeHtml(subtitle) + '</p>' +
    '</div></div></section>';
}

function renderMobilePrimaryHeader(title, rightMarkup, secondaryMarkup) {
  return '<section class="mobile-calendar-hero mobile-section-hero">' +
    '<div class="mobile-calendar-title-row mobile-section-title-row">' +
    '<div class="mobile-calendar-title-block">' +
    '<h2>' + escapeHtml(title) + '</h2>' +
    '</div>' +
    (rightMarkup ? '<div class="mobile-section-header-actions">' + rightMarkup + '</div>' : '') +
    '</div>' +
    (secondaryMarkup ? '<div class="mobile-section-secondary">' + secondaryMarkup + '</div>' : '') +
    '</section>';
}
...
function renderMobileFilterPanel() {
  return '<div class="mobile-filter-sheet">' +
    ...
    '<div class="form-row compact-row"><label for="mobile-filter-person">Group</label><input id="mobile-filter-person" type="text" value="' + escapeHtml(state.filters.person) + '" placeholder="Filter by group name"></div>' +
    ...
    '<label class="mobile-filter-toggle"><input id="mobile-filter-hide-archived" type="checkbox"' + (state.mobileHideArchived ? ' checked' : '') + '> <span>Hide past, cancelled, and completed</span></label>' +
    '</div>';
}

function renderMobileBookingCard(booking, includeSummary, showDayContext) {
  const approvalSummary = renderMobileApprovalMiniSummary(booking);
  const detailsText = getBookingDetailText(booking);
  const bookingTitle = booking.title || "Untitled booking";
  const isOwnedByCurrentGroup = isBookingOwnedByEffectiveGroup(booking);

  return '<article class="panel mobile-booking-card' + (isOwnedByCurrentGroup ? ' mobile-booking-card-owned' : '') + '" data-booking-select="' + booking.id + '">' +
    ...
    '<span class="status-badge ' + escapeHtml(getStatusClass(booking.status)) + '">' + escapeHtml(formatStatusLabel(booking.status)) + '</span>' +
    '</div>' +
    '<p class="mobile-booking-card-range"><span class="inline-icon">' + renderIcon("calendar") + '</span><span>' + escapeHtml(formatMobileBookingListRange(booking.start_time, booking.end_time)) + '</span></p>' +
...
function renderMobileSelectedDayBookingCard(booking) {
  const permissions = getEffectiveBookingPermissions(booking);
  const actionsMarkup = renderActionButtons(booking) + renderFlightCheckLink(booking);
  const canOpen = !isGuestView();

  return '<article class="panel mobile-day-booking-card" data-booking-select="' + booking.id + '">' +
    '<div class="mobile-day-booking-main">' +
    '<div class="mobile-day-booking-dot ' + escapeHtml(getStatusClass(booking.status)) + '"></div>' +
    '<div class="mobile-day-booking-copy">' +
    '<p class="mobile-day-booking-time">' + escapeHtml(formatSelectedDayTimeRange(booking.start_time, booking.end_time)) + '</p>' +
    '<h3>' + escapeHtml(booking.title || booking.area_name) + '</h3>' +
    '<p class="small-text">' + escapeHtml(booking.area_name) + '</p>' +
    '</div>' +
    '<div class="mobile-day-booking-side">' +
    '<span class="status-badge ' + escapeHtml(getStatusClass(booking.status)) + '">' + escapeHtml(formatStatusLabel(booking.status)) + '</span>' +
    (canOpen ? '<span class="mobile-day-booking-chevron">' + renderIcon("chevron") + '</span>' : '') +
    '</div>' +
    '</div>' +
    (actionsMarkup ? '<div class="mobile-day-booking-footer">' + actionsMarkup + '</div>' : '') +
    '</article>';
}
...
function renderMobileStatusLegend() {
  const items = [
  ...
  return items.map(function (item) {
    return '<span class="mobile-legend-item"><span class="mobile-legend-dot ' + escapeHtml(getStatusClass(item.key)) + '"></span>' + escapeHtml(item.label) + '</span>';
  }).join("");
}

function renderMobileScopeSwitch(kind, value, options) {
  return '<div class="view-switch mobile-inline-switch" data-mobile-scope-kind="' + escapeHtml(kind) + '">' +
    options.map(function (option) {
      return '<button type="button" class="toggle-button' + (value === option.value ? ' is-active' : '') + '" data-mobile-scope-kind="' + escapeHtml(kind) + '" data-mobile-scope-value="' + escapeHtml(option.value) + '">' + escapeHtml(option.label) + '</button>';
    }).join("") +
    '</div>';
}

function countBookingsWithConflict(bookings) {
  let conflictCount = 0;

  bookings.forEach(function (booking, index) {
    const bookingAreaIds = getBookingAreaIds(booking);
    const bookingStart = parseDateTime(booking.start_time).getTime();
    const bookingEnd = parseDateTime(booking.end_time).getTime();

    const hasConflict = bookings.some(function (otherBooking, otherIndex) {
      if (index === otherIndex) {
        return false;
      }

      const sameArea = getBookingAreaIds(otherBooking).some(function (areaId) {
        return bookingAreaIds.includes(String(areaId));
      });
      ...
      const otherStart = parseDateTime(otherBooking.start_time).getTime();
      const otherEnd = parseDateTime(otherBooking.end_time).getTime();
...
function getMobileNavIconName(viewName) {
  const iconMap = {
    calendar: "calendar",
    bookings: "bookings",
    create: "create",
  ...
  return iconMap[viewName] || "more";
}

function getMobileProfileBadgeText() {
  if (!state.currentUser) {
    return "G";
  }

  return String(state.currentUser.username || "U").charAt(0).toUpperCase();
}

function formatSelectedDayTimeRange(startValue, endValue) {
...
function getMobileSessionSubtitle() {
  if (!state.currentUser) {
  ...
  if (state.currentUser.role === "admin") {
    if (state.previewRole === "member") {
      return getEffectivePreviewGroupName();
    }
  ...
  return state.currentUser.group_name || "No group";
}
...
function renderMobileApprovalMiniSummary(booking) {
  if (booking.status === "planned") {
    return '<div class="mobile-approval-mini-row">' +
      '<span class="mobile-approval-mini-chip tone-planned"><span class="inline-icon">' + renderIcon("calendar") + '</span><span>Hold</span></span>' +
      '</div>';
  }
  ...
  const approvedCount = booking.approvals.length;
  const requiredCount = booking.required_approval_groups.length;
  const pendingCount = booking.pending_approval_groups.length;
  let markup = '<div class="mobile-approval-mini-row">';

  if (requiredCount > 0) {
    markup += '<span class="mobile-approval-mini-chip tone-approved"><span class="inline-icon">' + renderIcon("approval") + '</span><span>' + approvedCount + '/' + requiredCount + '</span></span>';
  }

  if (pendingCount > 0) {
    markup += '<span class="mobile-approval-mini-chip tone-requested"><span class="inline-icon">' + renderIcon("time") + '</span><span>' + pendingCount + ' pending</span></span>';
  } else if (requiredCount > 0) {
    markup += '<span class="mobile-approval-mini-chip tone-approved"><span class="inline-icon">' + renderIcon("approval") + '</span><span>Ready</span></span>';
  }
...
function renderFlightCheckLink(booking) {
  ...
  return '<a class="action-link-button" href="' + escapeHtml(getGoogleFlightsUrl(booking)) + '" target="_blank" rel="noopener noreferrer"><span class="inline-icon">' + renderIcon("plane") + '</span>Check flights</a>';
}
...
function formatMobileBookingListRange(startValue, endValue) {
  const startDate = parseDateTime(startValue);
  const endDate = parseDateTime(endValue);
  return formatLongDateShortYear(startDate) + " - " + formatLongDateShortYear(endDate);
}
...
async function submitMobilePasswordChange(currentPassword, newPassword) {
  try {
...
async function executeBookingAction(actionName, bookingId) {
  ...
  if (requiresActionConfirmation(actionName)) {
    const confirmed = window.confirm(getActionConfirmationText(actionName));
  ...
  try {
    ...
    applyOptimisticBookingAction(actionName, bookingId);
    renderPage();
...
async function handleMobileRootClick(event) {
  ...
  const scopeButton = event.target.closest("button[data-mobile-scope-kind][data-mobile-scope-value]");

  if (scopeButton) {
    const scopeKind = scopeButton.getAttribute("data-mobile-scope-kind");
    const scopeValue = scopeButton.getAttribute("data-mobile-scope-value");

    if (scopeKind === "bookings") {
      state.mobileBookingsScope = scopeValue;
    } else if (scopeKind === "approvals") {
      state.mobileApprovalsScope = scopeValue;
    } else if (scopeKind === "language") {
      state.language = scopeValue;
    }
...
async function handleMobileRootSubmit(event) {
  ...
  if (event.target.id === "mobile-password-change-form") {
    event.preventDefault();
    await submitMobilePasswordChange(
...
function handleMobileRootChange(event) {
  ...
  if (event.target.id === "mobile-filter-hide-archived") {
    state.mobileHideArchived = event.target.checked;
    renderPage();
  }
}
...
function getSortedTableBookings() {
  return sortBookingsList(state.filteredBookings);
}

function getMobileBookingsList() {
  const now = Date.now();

  return sortBookingsList(state.bookings.filter(function (booking) {
    const visibilityMatches = canBookingBeSeenInCurrentView(booking);
    const areaMatches = !state.filters.areaId || getBookingAreaIds(booking).includes(String(state.filters.areaId));
    const statusMatches = isGuestView() || !state.filters.status || booking.status === state.filters.status;
    const groupName = (booking.owner_group_name || "").toLowerCase();
    const groupMatches = !state.filters.person || groupName.indexOf(state.filters.person) !== -1;

    if (!visibilityMatches || !areaMatches || !statusMatches || !groupMatches) {
      return false;
    }

    if (state.mobileHideArchived) {
      if (booking.status === "completed" || booking.status === "cancelled") {
        return false;
      }

      if (parseDateTime(booking.end_time).getTime() < now) {
        return false;
      }
    }

    if (state.mobileBookingsScope !== "me" || !state.currentUser) {
      return true;
    }

    const previewGroupId = getEffectivePreviewGroupId();
    return Boolean(previewGroupId) && booking.owner_group_id === previewGroupId;
  }));
}

function isBookingOwnedByEffectiveGroup(booking) {
  const previewGroupId = getEffectivePreviewGroupId();

  if (previewGroupId) {
    return booking.owner_group_id === previewGroupId;
  }

  return false;
}

function sortBookingsList(bookings) {
  const sorted = bookings.slice();

  sorted.sort(function (bookingA, bookingB) {
...
function getMobileApprovalsList() {
  const previewGroupId = getEffectivePreviewGroupId();

  if (state.mobileApprovalsScope === "previous") {
    return getSortedTableBookings().filter(function (booking) {
      return booking.approvals.some(function (approval) {
        if (previewGroupId) {
          return approval.approver_group_id === previewGroupId;
        }

        return approval.approver_username === (state.currentUser ? state.currentUser.username : "");
      });
    });
  }
...
function applyOptimisticBookingAction(actionName, bookingId) {
  const bookingIndex = state.bookings.findIndex(function (item) {
  ...
  if (actionName === "approve") {
    booking.stored_status = "approved";
    booking.status = getDisplayStatus({ stored_status: "approved", end_time: booking.end_time });
    booking.pending_approval_groups = [];
  } else if (actionName === "reject") {
    booking.stored_status = "rejected";
    booking.status = "rejected";
  } else if (actionName === "cancel") {
    booking.stored_status = "cancelled";
...
function formatLongDateShortYear(dateValue) {
  const weekday = dateValue.toLocaleDateString(undefined, {
    weekday: "long"
  });

  return weekday + ", " + padNumber(dateValue.getDate()) + "." + padNumber(dateValue.getMonth() + 1) + "." + String(dateValue.getFullYear()).slice(-2);
}

function requiresActionConfirmation(actionName) {
  return ["approve", "reject", "cancel", "delete", "request"].includes(actionName);
}

function getActionConfirmationText(actionName) {
  const labels = {
    approve: "Approve this booking? This action cannot be undone easily.",
    reject: "Reject this booking? This action cannot be undone.",
    cancel: "Cancel this booking? This action cannot be undone easily.",
    delete: "Delete this booking? This action cannot be undone.",
    request: "Submit this planned booking into the request flow? This action should be treated as final for the current hold."
  };

  return labels[actionName] || "Are you sure? This action cannot be undone.";
}
...
function getNetworkFailureMessage() {
  ...
  return "Could not reach the backend. Make sure the API on " + API_BASE_URL + " is running.";
}
